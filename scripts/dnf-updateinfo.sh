#!/bin/bash
#####################################################################################
# dnf-updateinfo.sh - Retrieves available security and package updates using DNF
#                     and send it to Zabbix.
#
# Author: robin.roevens (at) disroot.org
# Version: 1.0
#
# Requires: dnf-plugin-security
#           zabbix-sender
#
...
#
# Host in Zabbix should be configured with the "Template Module DNF updateinfo by
...
ZBX_ITEM_PREFIX=dnf.updateinfo
ZBX_DATA=/tmp/zabbix-sender-dnf-updateinfo.data
...
echo "Retrieving DNF update summary..."
timeout -k 30 5m dnf updateinfo summary > $summary || { echo "Error: Failed retrieving updateinfo summary from DNF."; exit; }
...
CVES=$(timeout -k 30 5m dnf updateinfo list cves | grep "CVE-" | awk '{ print $1 }' | sort -u | sed -e ':a' -e 'N' -e '$!ba' -e 's/\n/, /g')

# Count total number of available updates
echo "Retrieving list of all updates..."
UPDATES=$(timeout -k 30 5m dnf -q check-update | egrep '(.i386|.x86_64|.noarch|.src)' | awk '{ print $1 }' | sort -u) 
if [[ $UPDATES ]]; then
